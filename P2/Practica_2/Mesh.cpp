...
void Mesh::CreateMesh(GLfloat* vertices, unsigned int* indices, unsigned int numOfVertices, unsigned int numberOfIndices)
{
	...
	/*
	Se van a crear dos shader uno que reciba una variable de entrada y otro que reciba dos variables
	*/
...
void MeshColor::RenderMeshColor() //Función de Render (Dibujado)
{
	...
	glDrawArrays(GL_TRIANGLES, 0, vertexCount); // si no se usan índices usa DrawArrays
	glBindVertexArray(0);
}
...
void Mesh::RenderMesh()
{
	...
	glBindVertexArray(VAO);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, IBO);
	glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_INT, 0); //si se usa índices se usa DrawElements
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
}
